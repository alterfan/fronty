...
class Split {
    ...
    renderGutter(index) {
        ...
        this.parent.insertBefore(gutter, this.splitedNodes[index + 1]);
    }
    changeDirection(newDirection) {
        if (this.direction == newDirection) newDirection = this.direction
        var sum = 0,
            gutters = document.querySelector(".split-gutter." + this.direction).parentNode.querySelectorAll(".split-gutter." + this.direction),
            old_sizes = getLocal((gutters.length + 1) + ".old." + this.direction) || [],
            axis = newDirection == HORIZONTAL ? "X" : "Y";
        for (let index = 0; index < (gutters.length + 1); index++) {
            old_sizes[index] = !old_sizes[index] ? this.options.parentSize / (gutters.length + 1) / this.options.parentSize * 100 : old_sizes[index];
        }
        for (var i = 0; i < gutters.length; i++) {
            ...
            addEventHandler(gutters[i].firstChild, "mousedown", (e) => {
                this.start(e);
                addEventHandler(document.getElementById("resizing-overlay"), "mousemove", (e) => {
        ...
        addEventHandler(window, "resize", (e) => {
            ...
            for (var n = 0; n < gutters.length; n++) {
                sum += old_sizes[n];
                moveGutter(gutters[n], this.gutterSize, axis, this.parent[newDirection == HORIZONTAL ? "offsetWidth" : "offsetHeight"] / 100 * sum)
            }
        });
