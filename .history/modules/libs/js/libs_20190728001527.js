    ...
    for (let i = 0; i < LINK.length; i++) {
        ...
        if (lib !== undefined) {
            for (let i = 0; i < libraries.link.length; i++) {
                console.log('lib[0] == libraries.link[i][0].replace(cdnjs, "./"): ', lib[0]бlibraries.link[i][0].replace(cdnjs, "./"));
                if (lib[0] == libraries.link[i][0].replace(cdnjs, "./")) lib[0].replace("./", cdnjs)
            }
