    ...
    switch (type) {
        case "script":
            for (let i = 0; i < libraries.script.length; i++) {
                var check = libraries.script[i][0].localeCompare(value);
                if (check === 0) value =libraries.script[i][0].replace("./", cdnjs);
            }
            $("#script-list").append(listItem(type, value, id));
            break;
        case "link":
            for (let i = 0; i < libraries.link.length; i++) {
                var check = libraries.link[i][0].localeCompare(value);
                if (check === 0) value = libraries.link[i][0].replace("./", cdnjs);
            }
