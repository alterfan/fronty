...
$(document).on("change", "[data-action=\"set-lib\"]", (e) => {
    ...
    var tag = $(e.target).parent().attr("data-tag"),
        ...
        url = $(e.target).val(),
        arr = $("[data-tag=\"" + tag + "\"]");
    url = url.replace(cdnjs, tag == "script" ? "./" : "");
    length = (arr.length);
...
function printList() {
    ...
    for (let i = 0; i < LINK.length; i++) {
        const lib = LINK[i];
        if (lib !== undefined) listItemAdd("link", cdnjs + lib[0], (i));
    }
    for (let i = 0; i < SCRIPT.length; i++) {
        const lib = SCRIPT[i];
        if (lib !== undefined) listItemAdd("script", cdnjs + lib[0], i);
    }
}
...
var listItem = (type, value, id) => {
    ...
    var datalist = "<datalist id=\"libraries-" + type + "\">" + optionInnetHTML + "</datalist>",
        input = $("<input>", {
            ...
            "list": "libraries-" + type,
            "data-action": "set-lib",
            "value": value === undefined ? "" : cdnjs + value,
            "placeholder": "type url, e.x.: " + cdnjs,
        });
