...
const libraries = {
    "link": [
        ["./libs/normalize/8.0.1/normalize.min.css", "Normalize", "8.0.1"],
        ["./libs/twitter-bootstrap/4.3.1/css/bootstrap.css", "Bootstrap", "4.3.1"],
        ["./libs/font-awesome/4.7.0/css/font-awesome.css", "Font Awesome", "4.7.0"],
        ["./libs/animate.css/3.7.2/animate.min.css", "Animate.css", "3.7.2"],
        ["./libs/material-design-icons/3.0.1/iconfont/material-icons.min.css", "Material design icons", "3.0.1"]
    ],
    "script": [
        ["./libs/jquery/3.4.1/jquery.min.js", "jQuery", "3.4.1"],
        ["./libs/jqueryui/1.12.1/jquery-ui.min.js", "jQuery UI", "1.12.1"],
        ["./libs/twitter-bootstrap/4.2.1/bootstrap.min.js", "Bootstrap", "4.2.1"],
        ["./libs/react/16.8.6/umd/react.production.min.js", "React", "16.8.6"],
        ["./libs/react-dom/16.8.6/umd/react-dom.production.min.js", "React-DOM ", "16.8.6"]
    ]
};
class Libraries {
    get usedLibs() {
...
var listItemAdd = (type, value, id) => {
    ...
    switch (type) {
        case "script":
            for (let i = 0; i < libraries.script.length; i++) {
                var check = libraries.script[i][0].localeCompare(lib[0]);
                if (check === 0) lib[0] == libraries.script[i][0].replace("./", cdnjs);
            }
            $("#script-list").append(listItem(type, value, id));
            break;
        case "link":
            for (let i = 0; i < libraries.link.length; i++) {
                var check = libraries.link[i][0].localeCompare(lib[0]);
                if (check === 0) lib[0] == libraries.link[i][0].replace("./", cdnjs);
            }
