...
class  Sidebar {
    ...
    render(side, parent, attachMode) {
        console.log('side, parent: ', side.length, parent);
        var that = this,
            $parent = $(parent);
        for (let i = 0; i < side.length; i++) {
            ...
            var $sidebar = $("<div>", {
                "id": "sidebar__" + s,
                "class": "fronty__workspace__sidebar fronty__workspace__sidebar__transition fronty__workspace__sidebar__" + s + " fronty__workspace__sidebar__" + (!this.attachMode ? "overlayed" : "splited")
            }).append($("<div>", {
