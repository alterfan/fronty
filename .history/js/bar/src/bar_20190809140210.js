((mod) => {
    ...
    let renderBar = (cm) => {
        ...
        cm.getScrollerElement().style["height"] = "calc(100% - 32px)";
        appendPanel(cm)
    }
    let appendPanel = (cm) => {
        ...
        for (var i = 0; i < btns.length; i++) {
            btns[i].onclick = function(event) {
                ...
                switch (a) {
                    case "clear":
                        actions.clear(activeInstance)
                    case "format":
                        actions.format(activeInstance)
                    case "undo":
                        actions.undo(activeInstance)
                    case "redo":
                        actions.redo(activeInstance)
                }
            }
        }
    }
    let hidePanel = (cm) => {
        ...
        b.querySelector(".bar__group__panel").classList.add("hidden");
        visiblity = false;
    }
    let showPanel = (cm) => {
        let b = cm.state.panels.wrapper.querySelector(".bar");
        b.querySelector(".bar__group__panel").classList.remove("hidden");
        activeInstance = cm;
        visiblity = true;
    }
