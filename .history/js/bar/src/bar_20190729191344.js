((mod) => {
    ...
    let close = (cm) => {
        let p = cm.state.panels.wrapper.querySelector(".bar__group__panel")
        console.log('cm.state.panels.wrapper: ', cm.state.panels.wrapper);
    ...
    CodeMirror.on(document, 'mousedown', (event) => {
        if (!isClosed && activeInstance !== null) {
            var e = cm.getWrapperElement().parentNode;
