((mod) => {
    ...
    let hidePanel = (cm) => {
        console.log('cm: ', cm.state.panels.wrapper.querySelector(".bar"));
        cm.state.panels.wrapper.querySelector(".bar__group__panel").classList.add("hidden")
        visiblity = false;
    }
    let showPanel = (cm) => {
        cm.state.panels.wrapper.querySelector(".bar__group__panel").classList.remove("hidden")
        cm.state.panels.wrapper.querySelector(".bar__group__panel").remove()
        visiblity = true;
    }
    CodeMirror.defineOption("toolBar", false, function(cm, val, old) {
        ...
        if (val) {
            ...
            CodeMirror.on(cm.getDoc(), "mousedown", (cm) => {
                if (activeInstance != cm) {
                    if (visiblity) {
                        hidePanel(cm)
                    }
                    activeInstance = cm;
                    showPanel(activeInstance)
                }
            });
        }
    });
