...
var sidebar = new class {
    ...
    open(el) {
        ...
        if (this.isClosed) {
            if (this.onTop) {
                ...
                $(sidebar).addClass("fronty__sidebar__attached")
            }
            setTimeout(function() {
                $(".fronty__overlay").addClass("visible");
                $(sidebar).addClass("visible").find('.fronty__sidebar__content').load('modules/' + view + '/' + view + '.html ');
            }, this.timeout);
        }
        this.view = view;
        this.isClosed = false;
    ...
    close(el) {
        ...
        $(".fronty__sidebar").removeClass('visible');
        setTimeout(() => {
            $(sidebar).find('.fronty__sidebar__content').html("");
