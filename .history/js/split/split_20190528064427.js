class Split {
    ...
    gutter() {
        for (let index = 0; index < this.split_elements_length - 1; index++) {
            this.split_elements[index].append(create("div", {
                class: "gutter gutter-" + this.split_direction
            }));
            switch (this.split_direction) {
                case "vertical":
                    this.split_elements[index].style.height = this.gutterSize;
                    this.split_elements[index].style.width = "100%";

                case "horisontal":
                    this.split_elements[index].style.width = this.gutterSize;
                    this.split_elements[index].style.height = "100%";
            }
        }
    }
    gutterStyles() {
