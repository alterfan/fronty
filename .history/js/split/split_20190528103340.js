...
class Options {
    ...
    get splitedChilds() {
        return this.parent.querySelectorAll(this.options.splitedChilds);
    }
    get direction() {
...
class Split extends Options {
    ...
    init() {
        ...
        let splitedChilds = this.getOption("splitedChilds"),
            length = splitedChilds.length;
        console.log('length: ', length);
        var elementSize = 100 / length;
        for (let index = 0; index < length; index++) {
            this.splitedChilds[index].style['flex-basis'] = "calc(" + elementSize + "%" + " - " + this.gutterSize + ")";
        }
