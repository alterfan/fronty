...
var pair = new class {
    ...
    set init(g) {
        ...
        this.p.offsetSize = this.isHorizontal ? this.p.node.offsetLeft : this.p.node.offsetTop;
    }
    get parentSize() {
...
class Split {
    ...
    start(e) {
        ...
        this.sizes = ls.getStorage(gutter.getAttribute("data-direction"));
    ...
    move(e) {
        ...
        let size = e[pair.clientAxis] - pair.p.offsetPos - pair.a.offsetPos,
            ...
            max = pair.a.offsetPos + pair.b.size + pair.b.size - correct;
        if (size < 0+this.minSizes) return
        if (size > max) return
        if ((size - correct) > pair.parentSize) return
        ...
        for (let i = 0; i < this.sizes.length; i++) {
            ...
            this.sizes[i] = getInt(element.style[pair.sizeAxis])
        }
        setLocal(pair.direction, this.sizes);
