...
class Split extends Options {
        ...
        for (let i = 0; i < gElement.length; i++) {
            ...
            g.addEventListener('mousedown', e => {
                ...
                g.addEventListener('mousemove', e => {
                    this.isMoving = true;
                    if (this.isMoving) console.log(x, y);
                });
    ...
    setStyles(gutter) {
        let _this = this,g=this.parent.querySelectorAll(this.options.childs);
            gSize = this.gutterSize,
            eSize = this.childSize;
        console.log('gSize.length: ', gSize);
        switch (_this.direction) {
            case V:
                for (let index = 0; index < gSize.length; index++) {
            ...
            case H:
                console.log('_this.childs[index]: ', gutter);
