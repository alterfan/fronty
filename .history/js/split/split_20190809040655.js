...
class Split {
    ...
    changeDirection(newDirection) {
        ...
        if (this.direction !== newDirection) {
            ...
            gutters = document.querySelectorAll(".split-gutter." + class_name_old);
            for (var i = 0; i < gutters.length; i++) {
                const g = gutters[i];
                console.log('gutters.length + 1: ', gutters.length + 1);
                sum = sum + this.parent[class_name == HORIZONTAL ? "offsetWidth" : "offsetHeight"] / 100 * getLocal(gutters.length + 1 + ".old." + this.direction)[i];

                console.log('sum: ', sum);
                g.style[this.Options.direction === HORIZONTAL ? "height" : "width"] = this.gutterSize + "px";
                g.style[this.Options.direction === HORIZONTAL ? "width" : "height"] = 100 + "%";
                g.style["transform"] = "translate" + (class_name == HORIZONTAL ? "X" : "Y") + "(" + sum + "px)";
                console.log(' g.style["transform"]: ', g.style["transform"]);
