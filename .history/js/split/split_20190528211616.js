...
class Split extends SplitRender {
    ...
    events() {
        ...
        for (let i = 0; i < gElement.length; i++) {
            ...
            gutter.addEventListener(MOUSE_EVENT.START, (e) => {
            ...
            doc.addEventListener(MOUSE_EVENT.MOVE, (e) => {
                if (this.isMoving) {
                    ...
                    var sumSize = parseFloat(this.childs[i].style["flex-basis"]) + parseFloat(this.childs[i + 1].style["flex-basis"]);
                    this.childs[i].style["flex-basis"] = parseFloat(this.childs[i].style["flex-basis"]) * basis + "%";
                    this.childs[i + 1].style["flex-basis"] = sumSize - (parseFloat(this.childs[i].style["flex-basis"]) * basis) + "%";
                }
            });
