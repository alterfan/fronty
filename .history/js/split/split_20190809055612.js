    ...
    changeDirection(newDirection) {
        ...
        if (this.direction !== newDirection) {
            ...
            for (var i = 0; i < gutters.length + 1; i++) {
                sum = sum + this.parent[class_name == HORIZONTAL ? "offsetWidth" : "offsetHeight"] / 100 * getLocal((gutters.length + 1) + "." + this.direction);
                el = document.querySelector("." + class_name_old + "-" + i);
                el.classList.remove(class_name_old + "-" + i);
                el.classList.add(class_name + "-" + i);
                resize(el, getLocal((gutters.length + 1) + "." + this.direction), "flex-basis");
                if (i < gutters.length) {
                    const g = gutters[i];
                    g.style["height"] = 100 + "%";
                    g.style["width"] = 100 + "%";
