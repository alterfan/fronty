    ...
    changeDirection(newDirection) {
        ...
        if (this.direction !== newDirection) {
            ...
            for (var i = 0; i < gutters.length; i++) {
                const n = i == gutters.length - 1 ? gutters.length : i;
                console.log('n: ', n);
                const g = gutters[i];
                const class_name = (newDirection == HORIZONTAL ? VERTICAL : HORIZONTAL) + "-" + n;
                console.log('class_name: ', class_name);
                g.className = "split-gutter " + newDirection;
                g.setAttribute("data-direction", newDirection);
                el = document.querySelector("." + class_name);
                el.classList.remove(class_name);
                el.classList.add(newDirection + "-" + n);
