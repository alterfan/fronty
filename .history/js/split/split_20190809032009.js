...
class Split {
    ...
    changeDirection(newDirection) {
        ...
        if (this.direction !== newDirection) {
            const class_name_old = (newDirection == HORIZONTAL ? VERTICAL : HORIZONTAL) + "-";
            const class_name = newDirection + "-";
            gutters = this.parent.querySelectorAll(".split-gutter." + (newDirection == HORIZONTAL ? HORIZONTAL : VERTICAL));
            for (var i = 0; i < gutters.length; i++) {
                const g = gutters[i];
                g.style[this.Options.direction === HORIZONTAL ? "width" : "height"] = this.gutterSize + "px";
                g.style[this.Options.direction === HORIZONTAL ? "height" : "width"] = "100%";
                g.classList.add(newDirection);
                g.classList.remove(newDirection == HORIZONTAL ? VERTICAL : HORIZONTAL);
                g.setAttribute("data-direction", newDirection);
                el = document.querySelector("." + class_name_old + i);
                if (el != null) el.classList.add(class_name + i);
                el.classList.remove(class_name_old + i);
                moveGutter(g, this.gutterSize, newDirection == HORIZONTAL ? "Y" : "X", this.parent[newDirection == HORIZONTAL ? "offsetHeight" : "offsetWidth"] / 100 * old_sizes[i]);
            }
