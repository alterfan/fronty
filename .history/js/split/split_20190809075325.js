...
class Pair {
    ...
    init(g) {
       ...
       // this.a.node.classList.add(this.g.node.getAttribute("data-direction") + "-" + (this.index - 1));
        this.b.node = this.g.node.nextElementSibling;
       // this.b.node.classList.add(this.g.node.getAttribute("data-direction") + "-" + (this.index));
...
class Split {
    ...
    changeDirection(newDirection) {
        ...
        for (var i = 0; i < gutters.length + 1; i++) {
            ...
            sum = sum + this.parent[newDirection == HORIZONTAL ? "offsetWidth" : "offsetHeight"] / 100 * old_sizes[i];
            if (i < gutters.length) {
                const g = gutters[i];
                ...
                addEventHandler(window, "resize", (e) => {
                    moveGutter(g, this.gutterSize, newDirection == HORIZONTAL ? "Y" : "X", (sum - this.gutterSize / 2))
                });
            }
        }
        ...
        setLocal((gutters.length + 1) + "." + this.direction, old_sizes);
        setLocal((gutters.length + 1) + ".old." + this.direction, old_sizes);
    }
    start(e) {
