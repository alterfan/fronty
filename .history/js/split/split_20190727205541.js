...
class Split {
    ...
    move(e) {
        if (!is.Move) return;
        let newSizeA, newSizeB
        this.oldSizeB = px2flex(pair.b.node[pair.offsetSize], this.parentSize, false);
        let calc = e[pair.clientAxis] - pair.a.offsetPos - pair.p.offsetPos - (this.gutterSize / this.splitedNodes.length);
        pair.g.node.offsetPos = pair.direction ? pair.g.node.offsetLeft : this.g.node.offsetTop;
        console.log(pair.g.node.offsetPos, this.oldSizeA, calc);
        newSizeA = px2flex(calc, this.parentSize, true);
        newSizeB = px2flex((pair.a.size - calc) + pair.b.size, this.parentSize, true);
        if (this.oldSizeA !== undefined && this.oldSizeA != pair.g.node.offsetPos) return
        resize(pair.a.node, newSizeA);
        resize(pair.b.node, newSizeB);
        this.oldSizeA = pair.a.node[pair.offsetSize];
    }
    done() {
