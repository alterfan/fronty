...
class Pair {
    ...
    init(g) {
        ...
        this.g.node.parentNode.querySelector("." + this.g.node.getAttribute("data-direction") + "-" + (this.index));
...
class Split {
    ...
    renderGutter(ghostPos, i, sum) {
        ...
        sizeGutter(gutter, this.options.direction, this.gutterSize);
    ...
    changeDirection(newDirection) {
        var el, precents= 0, sum = 0,
            overlay = document.getElementById("resizing-overlay"),
            parent = document.querySelector(".split-gutter." + this.direction).parentNode,
            gutters = parent.querySelectorAll(".split-gutter." + this.direction),
            old_sizes = getLocal((gutters.length + 1) + ".old." + this.direction),
            axis = newDirection == HORIZONTAL ? "X" : "Y";
        for (var i = 0; i < gutters.length + 1; i++) {
            el = document.querySelector("." + this.direction + "-" + i);
            el.classList.add(newDirection + "-" + i);
            if (this.direction !== newDirection) el.classList.remove(this.direction + "-" + i);
            if (i < gutters.length) {
                precents += old_sizes[i];
                //  sum += this.parent[newDirection == HORIZONTAL ? "offsetWidth" : "offsetHeight"] / 100 * old_sizes[i];
                gutters[i].setAttribute("data-direction", newDirection);
                gutters[i].className = "split-gutter " + newDirection;
                sizeGutter(gutters[i], newDirection, this.gutterSize);
                moveGutter(gutters[i], this.gutterSize, axis, this.parent[newDirection == HORIZONTAL ? "offsetWidth" : "offsetHeight"] / 100 * precents);
                addEventHandler(gutters[i].firstChild, "mousedown", (e) => {
                    this.start(e);
                    addEventHandler(overlay, "mousemove", (e) => {
                        mouseMoveHandler(this.move(e))
                    });
        ...
        addEventHandler(window, "resize", (e) => {
            precents = 0;
            for (var n = 0; n < gutters.length; n++) {
                precents += old_sizes[n];
                moveGutter(gutters[n], this.gutterSize, axis, this.parent[newDirection == HORIZONTAL ? "offsetWidth" : "offsetHeight"] / 100 * precents)
            }
        });
        setLocal((gutters.length + 1) + "." + newDirection, old_sizes);
        setLocal((gutters.length + 1) + ".old." + newDirection, old_sizes);
...
const sizeGutter = (g, newDirection, gutterSize) => {
    g.style["height"] = 100 + "%";
    g.style["width"] = 100 + "%";
    g.style[newDirection === HORIZONTAL ? "width" : "height"] = gutterSize + "px";
}
