...
class Split extends Options {
    ...
    events() {
        ...
        for (let i = 0; i < gElement.length; i++) {
            ...
            g.addEventListener('mousedown', e => {
                ...
                this.isMoving = true;
                console.log(parseFloat(this.childs[i].style["flex-basis"]));
                doc.addEventListener('mousemove', e => {
                    var basis = (e.clientX / this.childs[i].offsetWidth) * 100;

                    if (this.isMoving) {
                        this.childs[i].style["flex-basis"] = parseFloat(this.childs[i].style["flex-basis"]) * parseFloat(this.childs[i].style["flex-basis"]) + "%";
    ...
    setStyles(gutter) {
        let _this = this,
            g = this.parent.querySelectorAll(".gutter"),
