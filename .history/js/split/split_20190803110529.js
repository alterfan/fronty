...
var pair = new class {
    ...
    set init(g) {
        ...
        console.log('"." + this.g.node.getAttribute("data-direction") + "-" + (this.index - 1): ', "." + this.g.node.getAttribute("data-direction") + "-" + (this.index - 1));
...
class Split {
    ...
    renderGutter(ghostPos, i, sum) {
        ...
        addEventHandler(document, "mousedown", (e) => {
            this.start(e);
            addEventHandler(document, "mousemove", (e) => {
    ...
    start(e) {
        ...
        this.index = gutter.getAttribute("data-index");
        pair.a.size = pair.a.node[pair.offsetSize];
        console.log(' pair.a.size: ', pair.a.size);
        pair.b.size = pair.b.node[pair.offsetSize];
        console.log(' pair.b.size: ', pair.b.size);
        pair.parentSize = this.parentNode[pair.offsetSize];
    ...
    move(e) {
        ...
        if (size < 0 || (size - this.gutterSize / 2) > this.parentSize || size < this.minSizes) return
        let newSizeA = px2flex(size, this.parentSize, false);
        console.log('this.parentSize: ', pair.parentSize);
        let newSizeB = px2flex((pair.a.size - size) + pair.b.size, pair.parentSize, false);
        console.log((pair.a.size - size) + pair.b.size, newSizeB);
        resize(pair.a.node, newSizeA, pair.sizeAxis);
        resize(pair.b.node, newSizeB, pair.sizeAxis);
        moveGutter(pair.g.node, this.gutterSize, this.axis, pair.a.offsetPos + size);
    }
    done() {
        if (!is.Move) return;
        ...
        removeEventHandler(overlay, this.done);
