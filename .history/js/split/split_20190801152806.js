...
class Split {
    ...
    init() {
        ...
        if (this.splitedNodes !== null) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                сache[i] = size / this.parentSize * 100;
                resize(el, сache[i] + "%");
            }
            this.renderGutter(size);
            setLocal(this.options.direction, сache);
        }
    }
    renderGutter(ghostPos) {
        var gutter, gutterGhost,
            quantity = (this.splitedNodes.length - 1),
            overlay = document.getElementById("resizing-overlay");
        for (var i = 0; i < quantity; i++) {
            gutter = document.createElement("div");
            gutterGhost = document.createElement("div");
            gutterGhost.className = "split-gutter-ghost " + this.gutterClass;
            gutter.className = this.gutterClass;
            gutterGhost.setAttribute("data-index", i + 1);
            gutterGhost.setAttribute("data-direction", this.direction);
            gutterGhost.setAttribute("draggble", "true");
            gutterGhost.style[this.sizeAxis] = this.gutterSize + "px";
            gutterGhost.style[this.sizeAxis === "height" ? "width" : "height"] = "100%";
            gutter.style[this.sizeAxis === "height" ? "width" : "height"] = "100%";
            this.parentNode.insertBefore(gutter, this.splitedNodes[i + 1]);
            this.parentNode.insertBefore(gutterGhost, this.splitedNodes[i + 1]);
            moveGhost(gutterGhost, this.axis, ghostPos);
            addEventHandler(gutterGhost, "mousedown", (e) => {
                this.start(e);
    ...
    start(e) {
        ...
        pair.init = e.target.prevElementSibling;
        console.log('e.target.prevElementSibling: ', e.target.prevElementSibling.className);
    ...
    move(e) {
        ...
        moveGhost(document.querySelector(".split-gutter-ghost"), this.axis, calcA - (this.gutterSize / 2));
...
const moveGhost = (ghost, axis, pos) => {
    ghost.style['transform'] = translate(axis, pos - (ghost[axis == "Y" ? "offsetHeight" : "offsetWidth"] / 2));
}
