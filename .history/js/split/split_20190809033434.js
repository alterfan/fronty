...
class Split {
    ...
    changeDirection(newDirection) {
        var old_sizes, gutters, el, sum = 0,
            axis;
        ...
        if (this.direction !== newDirection) {
            ...
            for (var i = 0; i < gutters.length; i++) {
                const g = gutters[i];
                sum = sum + this.parent[newDirection == HORIZONTAL ? "offsetHeight" : "offsetWidth"] / 100 * old_sizes[i];
                axis = newDirection == HORIZONTAL ? "X" : "Y";
                g.style[this.Options.direction === HORIZONTAL ? "height" : "width"] = this.gutterSize + "px";
                g.style["translate" + (newDirection == HORIZONTAL ? "Y" : "X")] = "0";
