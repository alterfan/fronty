...
var pair = new class {
    ...
    init(g) {
        this.g.node = g;
        this.g.offsetPos = this.isHorizontal ? this.g.node.offsetLeft : this.g.node.offsetTop;
        console.log('this.g.offsetPos: ', this.g.offsetPos);
        ...
        console.log('this.p.offsetPos: ', this.p.offsetPos);
...
class Split {
    ...
    renderGutter(ghostPos, i, sum) {
        ...
        addEventHandler(window, "resize", (e) => {
            setTimeout(moveGutter(gutter, this.gutterSize, axis, this.parentNode[this.offsetSize] / 100 * sum));
        });
    }
    start(e) {
        ...
        pair.init(gutter);
        let cor = e[pair.clientAxis] - pair.g.offsetPos;
        console.log(e[pair.clientAxis], pair.g.offsetPos, cor);
    ...
    move(e) {
        ...
        let size = e[pair.clientAxis] - pair.p.offsetPos - pair.a.offsetPos,
            ...
            min = 0 + this.minSizes,
            max = pair.a.offsetPos + pair.a.size + pair.b.size - correct - this.minSizes;
        if (size < min) return
        if (size > max) return
        ...
        let sum = pair.sizes.reduce((first, last) => {
            return first + last
        })
        console.log(sum, pair.sizes);
        setLocal(pair.direction, pair.sizes);
