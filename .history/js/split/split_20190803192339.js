...
class Split {
   ...
   static destroy() {
        for (var i = 0; i < Split.splitedNodes.length; i++) {
            var el = Split.splitedNodes[i];
            el.classList.remove(Split.direction + "-" + i);
        }
        setLocal(Split.direction, []);
        console.log('this.direction: ', Split.direction);
        if (document.getElementById("resizing-overlay")) document.getElementById("resizing-overlay").remove();
        for (var i = 0; i < document.querySelectorAll(".split-gutter").length; i++) {
            var el = document.querySelectorAll(".split-gutter")[i];
            el.remove();
        }
        if (document.getElementById("resizing-overlay")) document.getElementById("resizing-overlay").remove();
        for (var i = 0; i < document.querySelectorAll(".split-gutter").length; i++) {
