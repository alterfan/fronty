...
class Split {
            ...
            for (var i = 0; i < n; i++) {
                ...
                if (i < n - 1) {
                    ...
                    addEventHandler(window, "resize", (e) => {
                        for (let i = 0; i < document.querySelectorAll("split-gutter-" + this.direction).length; i++) {
                            const gutter = document.querySelectorAll("split-gutter-" + this.direction)[i];
                            moveGutter(gutter, this.gutterSize, this.axis, this.parentNode[this.offsetSize] / 100 * sum);
                        }
                    });
                }
            }
    ...
    renderGutter(ghostPos, i) {
        ...
        addEventHandler(gutter, "dblclick", (e) => {
            let sum = 0;
            for (let n = 0; n < i + 1; n++) {
                sum += getLocal(this.direction)[n]
                console.log('sum: ', sum);
            }
            moveGutter(gutter, this.gutterSize, this.axis, this.parentSize / 100 * sum);
        });
    }
    start(e) {
