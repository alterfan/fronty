...
class Split extends Options {
    ...
    get gutter() {
        ...
        for (_i; _i < gutterQuantity; _i++) {
            ...
            _this.setStyles(_child, _gutter);
            _child.append(_gutter);
        }
        return true
    }
    setStyles(element, gutter) {
        let gSize, eSize, eQuantity, gQuantity;
        switch (this.direction) {
            case V:
                for (let index = 0; index < this.childsQuantity; index++) {
                    this.childs[index].style['flex-basis'] = eSize + "%";
                    this.childs[index].style['padding-bottom'] = eSize + "%";
                }
                element.style.paddingBottom = this.gutterSize;
                gutter.style.height = this.gutterSize;
                gutter.style.width = "100%";
            case H:
                element.style.paddingRight = this.gutterSize;
