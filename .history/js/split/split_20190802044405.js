...
class Split {
    ...
    start(e) {
        ...
        this.sizeAxis = pair.sizeAxis = pair.direction === horizontal ? "flex-basis" : "height";
        console.log('pair.sizeAxis: ', pair.sizeAxis);
    ...
    move(e) {
        ...
        if (!is.Move) return;
        var ofsetpos = e[pair.clientAxis] - pair.p.offsetPos;
        calcA = ofsetpos - pair.a.offsetPos;
        if (calcA < 0 || calcA < this.minSizes) return;
        if (ofsetpos <= pair.a.offsetPos) return;
        console.log(calcA, calcA < this.minSizes);
        ...
        moveGutter(pair.g.node, this.gutterSize, this.axis, ofsetpos);
...
const translate = (axis, pos) => {
    console.log("translate" + axis + "(" + pos + "px)");
