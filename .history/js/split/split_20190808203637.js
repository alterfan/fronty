...
class Split {
    ...
    changeDirection(direction) {
        var old_sizes = getLocal(this.direction);
        console.log(this.direction, old_sizes);
        var G = document.querySelectorAll(".split-gutter." + this.direction);
        for (var i = 0; i < G.length; i++) {
            const g = G[i];
            g.className = "split-gutter " + direction;
            g.setAttribute("data-direction", direction);
        }
        setLocal(direction, old_sizes);
        console.log('direction, old_sizes: ', direction, old_sizes);
