...
class Split {
    ...
    init() {
        ...
        if (this.splitedNodes !== null) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                resize(el, сache[i], this.direction === horizontal ? "width" : "height");
                if (i < n - 1) {
                    sum += сache[i]
    ...
    move(e) {
        ...
        if (!is.Move) return;
        document.getElementById("resizing-overlay").remove()
        for (let i = 0; i < pair.sizes.length; i++) {
            const element = this.splitedNodes[i];
            pair.sizes[i] = getInt(this.splitedNodes[i].style[pair.sizeAxis])
        }
        is.Move = false;
