...
class Sidebar {
    ...
    render(side, parent, attachMode) {
        ...
        for (let i = 0; i < side.length; i++) {
            const s = side[i];
            var attachMode = this.getCache(s) == undefined ? attachMode : this.getCache(s);
            var $sidebar = $("<div>", {
                "id": "sidebar__" + s,
                "class": "fronty__workspace__sidebar fronty__workspace__sidebar__transition fronty__workspace__sidebar__" + s + " fronty__workspace__sidebar__" + (!this.getCache(s) == undefined ? attachMode[i] : this.getCache(s) ? "overlayed" : "splited")
            }).append($("<div>", {
