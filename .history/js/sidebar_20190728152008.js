...
class Sidebar {
    ...
    init(attachMode) {
        ...
        console.log('$(".fronty__overlay"): ', this.parent.find(".fronty__overlay"));
        if (this.parent.find(".fronty__overlay") === undefined) this.parent.prepend($("<div>", {
            "class": "fronty__overlay",
            "data-action": "close-sidebar",
            "data-side": this.side
        }));
    ...
    static close(el) {
        ...
        setTimeout(() => {
            if (!this.cache) {
                $(".fronty__overlay").removeClass('visible');
                $("#sidebar__" + side).removeClass("visible");
                $("#sidebar__" + side).find('.fronty__workspace__sidebar__content').html("");
            }
        }, 300);
        Sidebar.view = "";
