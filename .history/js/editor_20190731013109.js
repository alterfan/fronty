var numPanels = 0;
class Editor {
    constructor(parent, index, mode, wrapper) {
        this.autoupdate = true;
        this.parent = document.getElementById(parent);
        this.cm;
        this.create(index, mode, wrapper)
    }
    create(index, mode, wrapper) {
        let textarea = document.createElement("textarea");
        textarea.setAttribute("data-index", index);
        textarea.setAttribute("id", mode + "_" + index);
        if (wrapper) {
            if (typeof wrapper === "string") wrapper = document.querySelectorAll(wrapper);
